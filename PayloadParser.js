function parseUplink(device, payload) {
    ...
    // Store Leak Flag
    if (decoded.Leak_flag != null) {
        var sensor2 = device.endpoints.byAddress("2");

        if (sensor2 != null)
            sensor2.updateIASSensorStatus(decoded.Leak_flag);
    };

    // Store Leak Count
    if (decoded.Leak_cnt != null) {
        var sensor3 = device.endpoints.byAddress("3");

        if (sensor3 != null)
            sensor3.updateGenericSensorStatus(decoded.Leak_cnt);
    };

    // Store Leak Flag
    if (decoded.Leak_time != null) {
        var sensor4 = device.endpoints.byAddress("4");

        if (sensor4 != null)
            sensor4.updateGenericSensorStatus(decoded.Leak_time);
    };
